...
#if 1
    // 验证拷贝是否成功
    // 将device_input_data拷贝回来主机内存，验证是否与host_input_data一致
    float *tmp_data = new float[3]{0, 0, 0};
    cudaMemcpy(tmp_data, device_input_data, input_data_size, cudaMemcpyDeviceToHost);
    for (int i = 0; i < 3; i++)
    {
        std::cout << "输入数据: " << tmp_data[i] << std::endl;
    }
    delete[] tmp_data;
#endif
