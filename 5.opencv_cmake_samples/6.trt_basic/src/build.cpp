...
void saveWeights(const std::string &filename, const float *data, int size)
{
    ...
    outfile.write((char *)(&size), sizeof(int));         // 保存权重的大小，参数是(取size变量的地址，size这个变量的大小)
    outfile.write((char *)(data), size * sizeof(float)); // 保存权重的数据，参数是(数据的指针，数据的大小)
    outfile.close();
}
// 读取权重
std::vector<float> loadWeights(const std::string &filename)
{
    ...
    int size;
    infile.read((char *)(&size), sizeof(int));                // 读取权重的大小，每次调用 write() 或 read() 函数，文件指针都会向后移动一定的距离。
...
int main()
{
    ...
    // 定义网络结构 mlp多层感知机：input(1,3,1,1) --> fc1 --> sigmoid --> output (2)
    // 创建一个input tensor ，参数分别是：name, data type, dims
    const int input_size = 3;
    nvinfer1::ITensor *input = network->addInput("data", nvinfer1::DataType::kFLOAT, nvinfer1::Dims2{1, input_size});  // 通过网络API构建
    ...
    // 读取权重
    auto fc1_weight_vec = loadWeights("model/fc1.wts");  // 返回值是一个vector
    auto fc1_bias_vec = loadWeights("model/fc1.bias");
#endif
    // 转为nvinfer1::Weights类型，参数分别是：data type, data, size
    nvinfer1::Weights fc1_weight{nvinfer1::DataType::kFLOAT, fc1_weight_vec.data(), (int64_t)(fc1_weight_vec.size())};
    nvinfer1::Weights fc1_bias{nvinfer1::DataType::kFLOAT, fc1_bias_vec.data(), (int64_t)(fc1_bias_vec.size())};

    // 调用addFullyConnected创建全连接层，参数分别是：input tensor, output size, weight, bias
    const int output_size = 2;
    nvinfer1::IConstantLayer* filterConst = network->addConstant(nvinfer1::Dims2{output_size, input_size}, fc1_weight);
    // nvinfer1::IFullyConnectedLayer *fc1 = network->addFullyConnected(*input, output_size, fc1_weight, fc1_bias);
    auto mm_layer = network->addMatrixMultiply(*input, nvinfer1::MatrixOperation::kNONE, *filterConst->getOutput(0), nvinfer1::MatrixOperation::kTRANSPOSE);

    auto biasConst_layer = network->addConstant(nvinfer1::Dims{2, {1, output_size}}, fc1_bias);  // 全连接层的偏置
    auto biasAdd = network->addElementWise(*mm_layer->getOutput(0), *biasConst_layer->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);

    // 添加sigmoid激活层，参数分别是：input tensor, activation type（激活函数类型）
    nvinfer1::IActivationLayer *sigmoid = network->addActivation(*biasAdd->getOutput(0), nvinfer1::ActivationType::kSIGMOID);
    ...
    // 设置最大工作空间大小，单位是字节
    config->setMaxWorkspaceSize(5 << 30); // 256MiB
