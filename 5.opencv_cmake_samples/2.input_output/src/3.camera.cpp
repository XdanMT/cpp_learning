...
// 定义命令行参数
DEFINE_int32(camera, 0, "Input camera"); // 摄像头编号0，这个可以从 /dev/video*来查看，可以打开笔记本自带的摄像头
...
int main(int argc, char **argv)
{
    ...
    int original_width = capture.get(cv::CAP_PROP_FRAME_WIDTH);
    int original_height = capture.get(cv::CAP_PROP_FRAME_HEIGHT);
    std::cout << "原始分辨率: " << original_width << "x" << original_height << std::endl;
    ...
    while (true)
    {
        ...
        capture.read(frame);
        std::cout << "读取的帧分辨率: " << frame.cols << "x" << frame.rows << std::endl;
        // capture >> frame;
        // flip
