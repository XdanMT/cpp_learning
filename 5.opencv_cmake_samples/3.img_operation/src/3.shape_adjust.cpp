...
int main()
{
    ...
    // 水平拼接，需要保证两张图片的高度（rows）一致，即高度240一致
    cv::Mat hconcat_img;
    cv::hconcat(cat_resize, dog_resize, hconcat_img);   // horizontal concatenation
    ...
    cv::Mat vconcat_img;
    cv::vconcat(cat_resize, dog_resize, vconcat_img);     // vertical concatenation
